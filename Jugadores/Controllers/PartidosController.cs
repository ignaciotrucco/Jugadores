...
public class PartidosController : Controller
{
    ...
    public JsonResult GuardarPartidos(int PartidoID, int JugadorID, DateOnly Fecha, decimal Minutos, string Estadio)
    {
        ...
        if (PartidoID == 0)
        {
            var existePartido = _context.Partidos.Where(e => e.FechaPartido == Fecha).Count();
            if (existePartido == 0)
            {
            ...
            {
                resultado = "Este partido ya existe";
            }
        }
        else
        {
            ...
            {
                var existePartidoEditar = _context.Partidos.Where(e => e.PartidoID != PartidoID && e.FechaPartido == Fecha).Count();
                if (existePartidoEditar == 0)
                {
